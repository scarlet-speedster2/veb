    ...
    if(!V){
        ...
        if(V->u > 2){
            if(V->cluster[high(x, V->u)] == NULL){
                V->summary = insert(V->summary, high(x, V->u), 1, sqrt(V->u));
            }
            V->cluster[high(x, V->u)] =
            insert(V->cluster[high(x, V->u)], low(x, V->u), qtd, sqrt(V->u));
        }
    }
    return V;
}

vEB* removeip(vEB *V, int x, int u){
    if(V->min == V->max){
        if(V->contMin > 1){
            V->contMin--;
            V->contMax--;
        ...
        if(x == V->min){
            if(!--V->contMin){
                if(V->summary){
                    ...
                    V->cluster[first_cluster] = removeip(V->cluster[first_cluster], new_min, sqrt(V->u));
                    if(V->cluster[first_cluster] == NULL)
                        V->summary = removeip(V->summary, first_cluster, sqrt(V->u));
                }
                else{
                    V->min = V->max;
                    V->contMin = V->contMax;
            ...
            if(!--V->contMax){
                if(V->summary){
                    ...
                    V->cluster[last_cluster] = removeip(V->cluster[last_cluster], new_max, sqrt(V->u));
                    if(V->cluster[last_cluster] == NULL)
                        V->summary = removeip(V->summary, last_cluster, sqrt(V->u));
                }
            }
        }
        else{
            V->cluster[high(x, V->u)] = removeip(V->cluster[high(x, V->u)], low(x, V->u), sqrt(V->u));
            if(V->cluster[high(x, V->u)] == NULL)
                V->summary = removeip(V->summary, high(x, V->u), sqrt(V->u));
        }
    }
    return V;
}
